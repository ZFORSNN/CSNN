...
class BaseNode(base.MemoryModule):
    ...
    def forward(self, x: torch.Tensor):
        ...
        * :ref:`中文API <BaseNode.forward-cn>`
        ...
        #print(spike)
        return spike
class BaseNode2(base.MemoryModule):
    def __init__(self, v_threshold: float = 1., v_reset: float = 0.,
                 surrogate_function: Callable = surrogate.Sigmoid(), detach_reset: bool = False,
                 counter=100,count_sum100=0,counter1=0):
        """
        * :ref:`API in English <BaseNode.__init__-en>`
        ...
        * :ref:`中文API <BaseNode.__init__-cn>`
        ...
        self.detach_reset = detach_reset
        self.surrogate_function = surrogate_function
        self.counter=100   #每一百次训练记录一次脉冲发放数
        self.counter1=0   #计数器，用于区分第一、二层的神经元
        self.count_sum100=0#记录总脉冲数量
    @abstractmethod
    def neuronal_charge(self, x: torch.Tensor):
        """
         * :ref:`API in English <BaseNode.neuronal_charge-en>`
    ...
    def neuronal_fire(self):
        ...
        * :ref:`中文API <BaseNode.neuronal_fire-cn>`
        ...
        return self.surrogate_function((self.v - self.v_threshold)*0.01)
    ...
    def neuronal_reset(self, spike):
        ...
        * :ref:`中文API <BaseNode.neuronal_reset-cn>`
        ...
        if self.v_reset is None:
            ...
            # hard reset
            self.v = (1. - spike_d) * self.v + spike_d * self.v_reset
            if torch.is_tensor(self.v) and torch.any(spike != 0).item():   #inhibition
                self.v=torch.where((spike==0) &(self.v>=(self.v_reset+50)), self.v-50,self.v)
            #print(spike_d)
            '''count=torch.sum(spike_d ==1).item()
            self.count_sum100+=count
            #print(count)
            #print(self.count_sum100)
            self.counter-=1
            if self.counter==1:
                self.counter=100    
                self.counter1+=1
                with open('output_data-inhibition.csv', mode='a', newline='') as file:
                    writer = csv.writer(file)
                    writer.writerow([self.counter1, self.count_sum100])
                if self.counter1==221:
                    self.counter1=0
                    self.count_sum100=0
                #print("神经元发放脉冲总数",self.count_sum100,end='\n')'''
    def extra_repr(self):
        return f'v_threshold={self.v_threshold}, v_reset={self.v_reset}, detach_reset={self.detach_reset}'
    ...
    def forward(self, x: torch.Tensor):
        ...
        * :ref:`中文API <BaseNode.forward-cn>`
        ...
        #print(spike.shape)
        return spike
class BaseNode22(base.MemoryModule):
    def __init__(self, v_threshold: float = 1., v_reset: float = 0.,
                 surrogate_function: Callable = surrogate.Sigmoid(), detach_reset: bool = False,counter=100,count_sum100=0,counter1=0):
        """
        * :ref:`API in English <BaseNode.__init__-en>`
    ...
    def neuronal_reset(self, spike):
        ...
        * :ref:`中文API <BaseNode.neuronal_reset-cn>`
        ...
        if self.v_reset is None:
            ...
            self.v = (1. - spike_d) * self.v + spike_d * self.v_reset
            if torch.is_tensor(self.v) and torch.any(spike != 0).item():   #inhibition
                self.v=torch.where((spike==0) &(self.v>=(self.v_reset+20)), self.v-20,self.v)
            #print(spike_d)
            count=torch.sum(spike_d ==1).item()
            self.count_sum100+=count
            #print(count)
            ...
            if self.counter==1:
                self.counter=100    
                self.counter1+=1
                with open('output_data2.csv', mode='a', newline='') as file:
                    writer = csv.writer(file)
                    writer.writerow([self.counter1, self.count_sum100])
                if self.counter1==221:
                    self.counter1=0
    ...
    def forward(self, x: torch.Tensor):
        ...
        * :ref:`中文API <BaseNode.forward-cn>`
        ...
        #print(spike.shape)
        return spike
class BaseNode3(base.MemoryModule):
    def __init__(self, v_threshold: float = 1., v_reset: float = 0., v_re: float = 1.,
                 surrogate_function: Callable = surrogate.Sigmoid(), detach_reset: bool = False,counter=100,count_sum100=0
        ...
        * :ref:`中文API <BaseNode.__init__-cn>`
        ...
        self.register_memory('v_threshold', v_threshold)
        self.register_memory('v_reset', v_reset)
        self.register_memory('v_re',v_re)
        self.refractory_period = refractory_period
        self.refractory_counter = torch.zeros(1)# 初始化不应期计数器
        self.detach_reset = detach_reset
        self.surrogate_function = surrogate_function
        self.counter=100
        self.count_sum100=0
    ...
    def neuronal_reset(self, spike):
        ...
        * :ref:`中文API <BaseNode.neuronal_reset-cn>`
        ...
        if self.v_reset is None:
            ...
            # hard reset
            self.v = (1. - spike_d) * self.v + spike_d * self.v_re
            if torch.is_tensor(self.v) and torch.any(spike != 0).item():   #inhibition
                self.v=torch.where(spike==0, self.v-20,self.v)
            #print(spike_d)
            count=torch.sum(spike_d ==1).item()
            self.count_sum100+=count
            #print(count)
            ...
            if self.counter==1:
                ...
                with open('output_data.csv', mode='a', newline='') as file:
                    writer = csv.writer(file)
                    writer.writerow([self.counter, self.count_sum100])
                print("神经元发放脉冲总数",self.count_sum100,end='\n')
            print(spike_d)
        self.refractory_counter = torch.where(spike > 0, self.refractory_period, self.refractory_counter)
        self.refractory_counter = torch.maximum(self.refractory_counter - 1, torch.zeros_like(self.refractory_counter))
        # hard reset
        #print(spike_d)
    def extra_repr(self):
        return f'v_threshold={self.v_threshold}, v_reset={self.v_reset}, detach_reset={self.detach_reset}'
    ...
    def forward(self, x: torch.Tensor):
        ...
        * :ref:`中文API <BaseNode.forward-cn>`
        ...
        """
        if self.refractory_counter.shape != x.shape:
        # 确保 refractory_counter 的形状与输入张量一致
            self.refractory_counter = torch.zeros_like(x)
        refractory_mask = self.refractory_counter > 0  # 生成一个布尔掩码
    
        # 更新处于不应期内的神经元
        if refractory_mask.any():  # 如果有任何神经元处于不应期
            # 对于不应期内的神经元，膜电位逐步降低
            self.v[refractory_mask] *= self.tau*(self.v-self.v_reset)+self.v_reset  # 膜电位降低到原来的90%，可以根据需求调整比例
            # 减少不应期计数器
            self.refractory_counter[refractory_mask] -= 1
    
        # 对不在不应期内的神经元，继续正常更新
        non_refractory_mask = ~refractory_mask  # 非不应期的神经元掩码
        spike = torch.zeros_like(x)
        if non_refractory_mask.any():  # 如果有任何神经元不在不应期内
            self.neuronal_charge(x[non_refractory_mask])  # 更新膜电位
            spike[non_refractory_mask] = self.neuronal_fire() # 计算脉冲
            spike[refractory_mask] = 0
            #print(f"spike shape: {spike.shape}, refractory_counter shape: {self.refractory_counter.shape}")
            #self.neuronal_reset(spike)  # 重置膜电位
    
            # 设置新的不应期计数器
            nonzero_indices = spike.nonzero(as_tuple=True)

            #assert all(idx.max() < dim_size for idx, dim_size in zip(nonzero_indices, self.refractory_counter.shape)), "Index out of bounds"
            self.refractory_counter[nonzero_indices] = self.refractory_period
...
class MTJ(BaseNode2):
    def __init__(self, tau: float = 2., v_threshold: float = 1.,
                 v_reset: float = 0., surrogate_function: Callable = surrogate.Sigmoid(),
                 detach_reset: bool = False,counter=100,count_sum100=0):
        """
        * :ref:`API in English <LIFNode.__init__-en>`
        ...
        * :ref:`中文API <LIFNode.__init__-cn>`
        ...
        super().__init__(v_threshold, v_reset, surrogate_function, detach_reset, counter,count_sum100=0)
        self.tau = tau
    ...
    def neuronal_charge(self, x: torch.Tensor):
        ...
        x=torch.where(x<=0,replacement,x)
        self.v=self.tau*(self.v + (1-self.tau)*((((1e12*x)**2/K))+(self.v_reset/self.tau)))
        #self.v=(1-self.tau)*((1e12*x)**2/K)*self.tau+self.v*(1+self.tau)-self.v_reset*self.tau
        #print(self.v[1,1].item())
        #self.v= torch.where(x<=Re, replacement, a)
        #print(x.shape)
class MTJ22(BaseNode22):  #用于数据监控
    def __init__(self, tau: float = 2., v_threshold: float = 1.,
                 v_reset: float = 0., surrogate_function: Callable = surrogate.Sigmoid(),
                 detach_reset: bool = False,counter=100,count_sum100=0):
        """
    ...
    def neuronal_charge(self, x: torch.Tensor):
        ...
        #self.v= torch.where(x<=Re, replacement, a)
        #print(x.shape)
class MTJ2(BaseNode3):#用于不应期
    def __init__(self,v_re:float = 1. , tau: float = 2., v_threshold: float = 1.,
                 v_reset: float = 0., surrogate_function: Callable = surrogate.Sigmoid(),
                 detach_reset: bool = False,counter=100,count_sum100=0
                 ,refractory_period: int = 0):
        """
        ...
        * :ref:`中文API <LIFNode.__init__-cn>`
        ...
        super().__init__(v_threshold, v_reset,v_re, surrogate_function, detach_reset,counter,count_sum100=0)
        self.tau = tau
    #v_re用于设置不应期
    def extra_repr(self):
        return super().extra_repr() + f', tau={self.tau}'
    ...
    def neuronal_charge(self, x: torch.Tensor):
        ...
        replacement=torch.zeros_like(x)
        x=torch.where(x<=0,replacement,x)
        self.v=self.tau*(self.v + (1-self.tau)*((((0.7*1e12*x)**2/K))+(self.v_reset/self.tau)))
